...
def build_dataset(cfg):
    ...
    if cls == "FB15k-237" or cls=="WN18RR" or cls=="NELL-995":
        dataset = datasets.DistinctiveTransRelLinkPredDataset(name=cls, **cfg.dataset)
...
def build_distinctive_dataset(cfg,device,accuracy_threshold,recall_threshold):
    cls = cfg.dataset.pop("class")
    if cls == "FB15k-237" or cls=="WN18RR" or cls=="NELL-995":
        dataset = datasets.DistinctiveTransRelLinkPredDataset(name=cls, device=device,
                                                            accuracy_threshold=accuracy_threshold,
                                                            recall_threshold=recall_threshold, **cfg.dataset)
    elif cls.startswith("Ind"):
        dataset = datasets.DistinctiveIndRelLinkPredDataset(name=cls[3:],device=device,accuracy_threshold=accuracy_threshold,recall_threshold=recall_threshold, **cfg.dataset)
    else:
        raise ValueError("Unknown dataset `%s`" % cls)
